...
# Predict Upcoming Intraday Prices using Weighted Moving Average
def predict_intraday_prices():
    ...
    sma_value = data.iloc[-1]["SMA"]
    ema_value = data.iloc[-1]["EMA"]

    # Predict next 10 minutes using the weighted average of SMA, EMA, and latest price
    future_predictions = []
    for i in range(1, 11):
        predicted_price = (latest_price * 0.4) + (sma_value * 0.3) + (ema_value * 0.3)
        future_predictions.append({
            "time": (datetime.now() + timedelta(minutes=i)).strftime("%H:%M"),
            "predicted_price": round(predicted_price, 2)
        })
        # Simulating next step by updating the latest values
        latest_price = predicted_price
        sma_value = (sma_value + predicted_price) / 2
        ema_value = (ema_value * 0.8) + (predicted_price * 0.2)
